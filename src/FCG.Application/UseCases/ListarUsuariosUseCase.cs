...
public class ListarUsuariosUseCase(IUsuarioRepository repositorio)
{
    ...
    {
        (IReadOnlyList<Usuario> items, int total) = await repositorio
            .ListarPaginadoAsync(pagina, tamanhoPagina, cancellationToken);

        List<UsuarioResponse> responses = items
            .Select(u => new UsuarioResponse(
                u.Id, u.Nome, u.Email.Endereco, u.Tipo.ToString(),
                u.DataCriacao, u.Ativo))
            .ToList();
